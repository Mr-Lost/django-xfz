var xfzalert = {
    ...
    swal(text);
    swal(title, text);
    swal(title, text, icon); icon=['warning', 'info', 'success', 'error']

    buttons: true;显示两个按钮
    buttons: false;隐藏两个按钮 (默认只显示确认按钮)
    buttons: 'ConfirmText';
    ...
    */
    'alertConfirm': function (params) {
        swal({
            ...
            buttons: [params['cancelText']?params['cancelText']:'取消', params['confirmText']?params['confirmText']:'确认'],
            content: {
                attributes: {
    ...
    'alertOneInput': function (params) {
        swal({
            ...
            content: {
                element: 'input',
                attributes: {
                    placehodler: params['placeholder']?params['placeholder']:'',
                    type: 'input',
            ...
            buttons: {
                cancel: {
                    text: params['cancelText']?params['cancelText']:'取消',
                    visible: true,
                },
                confirm: {
                    text: params['confirmText']?params['confirmText']:'确认',
                    closeModal: false,
                }
            },
        }).then((result) => {
            var inputValue = $('.swal-content').find("input[class='swal-content__input']").val();
            if(result){
                if(inputValue === ''){
                    swal('提示', '输入框不能为空！', 'warning');
                    return false;
                }
                if(params['confirmCallback']){
            ...
            // var content = $('.swal-content');
            // var inputValue = content.find("input[class='swal-content__input']").val();
            // var confirmBtn = $('.swal-button--confirm');
            // var cancelBtn = $('.swal-button--cancel');
